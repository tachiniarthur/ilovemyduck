  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-4xl bg-gradient-to-br from-duck-100 to-pond-300/40 p-6 text-center shadow-soft ring-1 ring-duck-200">
        ...
        <h2 className="mt-2 font-display text-2xl font-extrabold text-bill-600">
          Seus patinhos estão prontos! 🎉
        </h2>
        <p className="font-body text-sm text-duck-700/80">
      ...
      <div className="flex flex-col gap-2 sm:flex-row">
        {env.canShare && (
          <button
            ...
            className="flex-1 rounded-2xl bg-pond-500 px-4 py-3 font-display text-sm font-extrabold text-white shadow-pop transition-all hover:bg-pond-400 active:translate-y-0.5 active:shadow-none"
          >
        ...
        <button
          ...
          className="flex-1 rounded-2xl bg-sky-500 px-4 py-3 font-display text-sm font-extrabold text-white shadow transition-all hover:bg-sky-400 active:translate-y-0.5 disabled:cursor-wait disabled:opacity-80"
        >
          {zipProgress !== null
...
function SaveTip({
  ...
  return (
    <div className="flex items-start gap-3 rounded-3xl bg-sky-200/60 px-4 py-3">
      <span className="text-lg" aria-hidden>
        💡
      </span>
      <p className="font-body text-sm text-sky-500">{text}</p>
    </div>
  );
}
